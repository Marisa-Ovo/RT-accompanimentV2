...
class PianoDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            labeler: playing token 标签生成策略（None 时默认全部 PLAY）
                     仅当 config.use_playing_token=True 时生效
        """
        ...
        # Playing Mode 配置
        self.use_playing_token = getattr(config, "use_playing_token", False)
        self.play_token_id = getattr(config, "play_token_id", 268)
        self.skip_token_id = getattr(config, "skip_token_id", 269)
        self.labeler = labeler
        if self.use_playing_token:
            print(
                f"Playing Mode 已启用（play={self.play_token_id}, skip={self.skip_token_id}）"
                f"  labeler: {type(labeler).__name__ if labeler else 'None（默认全 PLAY）'}"
            )
    ...
    def __getitem__(self, idx):
        ...
        for i in range(num_measures):
            ...
            part0_beats, part1_beats = process_measure_with_beat_interleaving(
                measure, tokenizer=self.tokenizer, timesteps_per_beat=4
            )
        ...
        all_input_tokens = []
        all_label_tokens = []
        beat_global_idx = 0  # 真实 beat 的全局计数（不含 bar/pad）

        for p0, p1 in zip(part0_padded, part1_padded):
            is_pad_beat = len(p1) == 1 and p1[0].item() == pad_marker
            is_bar_beat = len(p1) == 1 and p1[0].item() == self.bar_token
            ...
            # Playing Mode：仅对真实 beat（非 bar、非 pad）插入 playing token
            if self.use_playing_token and not is_pad_beat and not is_bar_beat:
                if self.labeler is not None:
                    ctx = LabelContext(
                        beat_index=beat_global_idx,
                        part0_beat_tokens=p0.tolist(),
                    )
                    playing_label = self.labeler.label_beat(p1.tolist(), ctx)
                else:
                    playing_label = 1  # 默认 PLAY（未设置 labeler 时）
                playing_id = self.play_token_id if playing_label == 1 else self.skip_token_id
                playing_tensor = torch.tensor([playing_id], dtype=torch.long)
                all_input_tokens.append(playing_tensor)
                all_label_tokens.append(playing_tensor)  # playing token 参与 loss
                beat_global_idx += 1
    ...
    def __call__(self, features: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        for feature in features:
            ...
            if seq_len < max_len:
                ...
                input_ids = torch.cat([input_ids, torch.full((padding_len,), self.pad_token_id, dtype=torch.long)])

                labels = torch.cat([labels, torch.full((padding_len,), -100, dtype=torch.long)])

                attention_mask = torch.cat(
                    [torch.ones(seq_len, dtype=torch.long), torch.zeros(padding_len, dtype=torch.long)]
                )
            else:
